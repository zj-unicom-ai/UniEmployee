<template>
  <div class="im-page">
    ...
    <n-modal v-model:show="showForm" preset="card" :title="editing ? '编辑频道' : '新建频道'" style="width:520px"><n-form>
      ...
      <template v-if="form.provider === 'feishu'"><n-divider>飞书凭证</n-divider><n-form-item label="App ID"><n-input v-model:value="form.app_id" /></n-form-item><n-form-item label="App Secret"><n-input v-model:value="form.app_secret" type="password" show-password-on="click" /></n-form-item><p class="hint">Tenant Key 将在收到飞书事件后自动识别。</p></template>
...
const form=reactive({name:'',description:'',provider:'web',enabled:true,app_id:'',app_secret:''})
...
function openCreate(){editing.value=null;Object.assign(form,{name:'',description:'',provider:'web',enabled:true,app_id:'',app_secret:''});showForm.value=true}
function edit(i){editing.value=i;Object.assign(form,{name:i.name,description:i.description||'',provider:i.provider,enabled:i.enabled,app_id:'',app_secret:''});showForm.value=true}
async function save(){if(!form.name.trim())return;saving.value=true;try{const u=editing.value?`/im/channels/${editing.value.id}`:'/im/channels';const saved=await api[editing.value?'put':'post'](u,{name:form.name,description:form.description,provider:form.provider,enabled:form.enabled});const channelId=editing.value?.id||saved.data.id;if(form.provider==='feishu'&&form.app_id&&form.app_secret)await api.put(`/im/channels/${channelId}/credentials`,{app_id:form.app_id,app_secret:form.app_secret});showForm.value=false;await load()}catch(e){error.value=e.response?.data?.detail||'保存失败'}finally{saving.value=false}}
