...
- **HTML 看板纳入产物工作区**：对话内通过 `REPORT_HTML_START/END` 输出的完整 HTML 看板，在新生成时自动保存并登记为会话产物；打开历史对话时幂等补登记，保留原有对话内预览，并支持在产物工作区查看、下载和按现有权限共享
- **历史看板批量回填**：新增 `scripts/backfill_inline_reports.py`，可预览并批量归档历史对话里的助手看板内容；忽略工具消息，避免把技能模板或工具输出重复登记
- **小数工作台体验对齐**：定制分析对话复用通用工作台的历史会话侧栏、执行详情面板和标准宽度/全宽模式，同时保留数据源、模型、SQL 展示和表格问答能力

### 优化

- 产物工作区增加分页，每页显示 15 项
- 对话中已归档的 HTML 看板显示工作区入口；常规文件继续使用原有文件卡片
- `conversation_files` 增加 `artifact_type`，既有数据库启动时幂等迁移

### 验证

- `npx vite build` 通过；Python 文件语法编译通过；本地 `/health` 检查通过
